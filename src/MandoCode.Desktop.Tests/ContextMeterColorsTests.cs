...
/// <summary>
/// The context meter's bar is read by its length, so in every theme the filled part must clear the
/// non-text floor against both the page and the empty track. A theme added with a faint green,
/// gold, or red fails here instead of shipping a bar that looks full, or empty, when it isn't.
/// </summary>
public class ContextMeterColorsTests
{
    ...
    public void FillStandsApartFromThePageAndTheTrack(string name, ContextMeter.Level level)
    {
        ...
        Assert.True(onPage >= ContextMeterColors.NonTextFloor, $"{name}/{level}: fill is {onPage:F2}:1 against the page");
